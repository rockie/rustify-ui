---
name: dev-plan
description: 基于用户需求、仓库规则和当前代码事实，撰写或评审可直接交给实现者执行、跨对话续做的开发计划、技术设计、立项方案与实施路线图。用户要求先规划后实施、写 PLAN/设计方案、评审既有开发计划时使用。按既有计划实施、续做或回写实施进度时不要使用——计划文件自带实施约定，直接读计划；用户只要求直接实现且没有计划文件时也不要使用。
---
...
按既有计划实施或续做不用本 skill：计划顶部的「实施者定位」与「实施进度」已带齐实施需要的全部约定（进度回写、独立记录格式、subagent 并行、进度校验命令），实施者直接读计划。同一对话里写完计划接着实施，也按计划顶部两节执行，不再读本 skill 的参考。由此推出一条写作约束：**实施期需要的任何规则都必须写进骨架的这两节**，不能只留在本 skill 里。
...
- 骨架顶部的“实施者定位”与“实施进度”协议原样带入计划，不改写、不删减：它们是给执行计划的 agent 的完整合同，与写计划的架构师定位是两回事。唯一的填空是“回写后自查”里的 `<进度校验命令>`：本 skill 目录在目标仓内时，换成从仓库根可直接运行的 `bash <本 skill 目录相对仓库根的路径>/scripts/check_progress.sh <计划相对仓库根的路径> .`；不在仓内（全局安装）时把整句改成“每次回写后按上述条目人工核对”，不写本机绝对路径。
...
5. 检查“实施者定位”与“实施进度”的回写协议为骨架原文在位、`<进度校验命令>` 已替换；每个里程碑的退出条件都以「回写『实施进度』」结尾，依赖与可并行边界明确；跑 `"$SKILL_DIR/scripts/check_progress.sh" <计划文件> <仓库根>` 确认进度与里程碑一致，记录链接有效；初稿不预填完成事实，不把实现与验收详情写回计划。
