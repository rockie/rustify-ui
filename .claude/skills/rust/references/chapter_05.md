...
```rust
use std::assert_matches; // ❗ the macro lives at the root of `std`, one `use` per macro

assert_matches!(error, MyError::BadInput(_));
```
> The pre-stabilization path `use std::assert_matches::assert_matches;` was a *module*, and it does **not** exist on stable - it fails with `E0432: could not find assert_matches in std`. Either import the macros (`use std::assert_matches;`, `use std::debug_assert_matches;`) or call them fully qualified as `std::assert_matches!(..)`.
