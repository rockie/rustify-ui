...
* [`smallvec`](https://docs.rs/smallvec/latest/smallvec/) is for the *inverse* case: a `Vec`-like type that keeps **small** payloads inline on the stack and spills to the heap only once it outgrows the inline capacity. Reach for it when most values are small and a few are not - not as a way to heap-allocate large arrays.
