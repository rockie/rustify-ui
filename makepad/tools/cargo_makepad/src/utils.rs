...
/// Every dependency of `build_crate` for `target` mapped to its source
/// directory. Path dependencies come straight from `cargo tree`; registry
/// crates are located through the `<name>.path` file their build script
