...
| R15 / R25 semantics | partial | Eighteen categories with name, role, value and state; the four floating ones rebuilt onto the overlay stack with focus trapping and return; the user reported the dialog VoiceOver walkthrough passed on 2026-09-11 and waived the untested remainder; full screen-reader support is not verified | [M2](../../validation/p2/m2.md), [accessibility](accessibility.md) |
...
| R17 languages and Unicode | partial | The SDK's own words in two languages, `Intl` formatting with the application's options, twenty B5 samples drawn by both halves. Text-sample acceptance has a user-reported pass dated 2026-09-11; the reference image and per-sample comparisons were not supplied, and the GPU Arabic/Hebrew font gap remains | M7 |
...
| R35 accessibility numbers | partial | The contrast ratios are now a host test over both themes; contrast and zoom acceptance has a user-reported pass dated 2026-09-11, without separate results for each page, theme or zoom level | [accessibility](accessibility.md) |
...
| R40 acceptance | partial (P2 closed; full PRD deferred) | `cargo xtask verify --suite p2`, two clean builds of each of three examples, these six reports and this matrix; VoiceOver has a dialog pass plus a user waiver for the remainder; the other manual items have user-reported passes (2026-09-11), with detailed observations not supplied | [M8](../../validation/p2/m8.md) |
...
| R-6 languages and samples | §5.6 | Delivered with known font limitations; samples accepted by the user, reference images and individual comparisons not supplied |
| R-7 acceptance | §10 | The automated half is delivered; remaining VoiceOver checks are user-waived after a dialog pass; the other manual items have user-reported passes (2026-09-11), with detailed observations not supplied |
...
| NFR-3 compatibility | §9.4 | Chrome is the gate; VoiceOver has a dialog pass plus a user waiver; the other manual items have user-reported passes (2026-09-11), with detailed observations not supplied |
...
| A-4 acceptance resources | §0.2 | **Closed for P2, 2026-09-11.** VoiceOver dialog passed and the remainder was user-waived; pinyin, samples, and contrast and zoom received user-reported passes. Detailed observations not supplied and existing font limitations remain |
