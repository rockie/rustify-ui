...
| NFR-1 | 性能；R29/R30（B1 部分） | B1 真实负载的冷/热启动、首次可用压缩体积、跨区选择与属性编辑输入到呈现延迟 p95/p99；**2026-09-11 起是通过门**：R29 AC1/AC2/AC3 与 R30 AC1 的 B1 数值由 `tests/browser/budgets.ts` 持有、由 `--project=budget` 断言 | §7，V11，M9 | **预算已批准（A-3 关闭）**；门的边界见 §7 |
...
| --- | --- | --- | --- |
| NFR-1 | **通过门**（A-3 于 2026-09-11 关闭） | B1 冷启动 30 次 p95 ≤ 5 s、热启动 30 次 p95 ≤ 2 s、首次可用**压缩**传输 ≤ 8 MiB 且漏计文件 0、跨区选择与属性编辑 ≥1,000 次 p95 ≤ 50 ms 且 p99 ≤ 100 ms 且错误动作 0；数值只写在 `tests/browser/budgets.ts`，由 `--project=budget` 断言，页内测量（2.5 s/3 MiB/1 s 是 B0 的值，不适用） | 超出即 `--project=budget` 失败，`verify --suite p2` 随之失败，M9 不得记完成 |
...
**门的边界**（写在这里是因为一个被当成「全都测过」的门比没有门更糟）：这套门跑在**一台机器、一个浏览器**（macOS Chrome，WebGL2 由 SwiftShader 软栅格化）上，R29 要求的「每个性能机与浏览器组合各自达标」只覆盖了这一组；**B0 与 B2/B3 不在本期**，R30 AC2/AC3 按未测记；冷启动门跑在**回环**上，量的是这台机器的代价而不是链路的代价，限速链路的数字仍留在 `m8-network.spec.ts` 作观察值，且那里服务端不压缩，因此不是 AC2 的读数。
...
| V11 | B1 冷启动 30 次、热启动 30 次（均页内计时，失败启动单独计）；首次加载逐文件压缩后求和并分类；≥1,000 次跨区选择/属性编辑（区域点击→DOM 显示、DOM 编辑→区域呈现交替） | 冷 p95 ≤ 5 s、热 p95 ≤ 2 s、压缩 ≤ 8 MiB 且无未分类文件、p95 ≤ 50 ms 且 p99 ≤ 100 ms、拒绝与错误 0、失败启动 0；超出即失败 | NFR-1；M8 基线 / **M9 门** |
...
| `npx playwright test --project=budget` | M9：R29/R30 的 B1 通过门（30 次冷启动 + 30 次热启动 + 首次加载压缩体积 + 1,000 次跨区动作），单独一个 project 因为它慢 |
...
依赖链 M1 → M2 → M3 → M4 → M5 → M6 → M7 → M8 → M9 → M10。M4 与 M6 各以探针开头，探针失败先重审 ADR 再继续。M9/M10 是 M8 之后按用户 2026-09-11 的两条指示新增的，不改前八个里程碑的已完成事实。
...
- PRD 的 B1 是负载定义而非预算批准；只有夹具逐项匹配 §5.2 后才能用 B1 名称。（预算已于 2026-09-11 批准，见 A-3/M9。）
- `makepad-stitch` 是线程码解释器：每条指令尾调用下一条，只有优化过的构建才把它们变成跳转。opt-level 0 下每条**执行过的**指令都留一帧栈，于是任何几千次迭代的 wasm 循环都会爆栈——而且 abort 会连带杀掉整个测试二进制，报错只点名第一个跑到那儿的用例（`math_aot::batch_edges`），看起来像单条用例的问题。stitch 自己的 `Cargo.toml` 写了 `[profile.dev] opt-level = 1`，但那是它作为工作区根时才生效，路径依赖的消费方必须自己写一遍。
- 文本缓存（shaper 与 layouter 各 4,096 条）会在「每次动作都画一段没画过的字」的负载下一直填。一千个对象的名字、位置和尺寸就是三千条第一次，而两分钟 10 Hz 的耐久跑**还不到一圈**：测那种跑的尾部，测到的是缓存在填，不是内存在漏。耐久测试必须先不限速走完一圈工作集，再开始计时。
...
| R29/R30 | R29 三条与 R30 AC1 的 **B1 列是通过门**（M9）；B0 列与 R30 AC2/AC3（B2/B3）不在本期，记未测 | NFR-1，M8/M9 | V11；门 |
