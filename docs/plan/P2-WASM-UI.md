...
- 下一步：**只剩人工验收**。四份**表单已经备好**（`docs/validation/p2/manual/`，含 §9.4 要求的设备/OS/浏览器/辅助技术/build id 表头、步骤、要看什么、结果表），需要用户做的是**做那四场会话并填进去**，不是从零写文档。每份表单带 `STATUS: NOT PERFORMED`，`verify --suite p2` 见到这行就仍按缺失计（打印 `form` 而不是 `held`）——**空表单不能读成通过**，这是那条命令存在的全部理由。四份是：
  1. `voiceover.md`——**屏幕阅读器实际念出来的内容**；底下的无障碍树已由 `p2-semantics` 与新增的 `p2-a11y` 覆盖：作用域内每个可操作控件都有名字（读整棵树，不是逐个点名）、20 条导航名字互不相同、从头 Tab 300 次会绕回来（没被卡住）、模态打开时 20 个 Tab 停留点一次都进不到它后面的作用域内容——最后这条是 `p2-semantics` 做不到的，因为「Escape 能关」坏掉的焦点陷阱也能通过。
  2. `pinyin.md`——**真实输入法会话本身**；底下的机制已由 `p2-ime` 覆盖（组合中不算值、组合期间的按键属于组合、组合结束才提交），**而且写这条用例当场抓到一个真缺陷**：输入「工作」的过程中应用已经把 `gongzuo` 存成了对象名，放弃候选也留在那里；两套文本组件（SDK 的与组件 crate 的）都有同一个毛病，都已修。
  3. `samples.md`——**方向与顺序**对照参考渲染逐条判（**参考渲染图需要评审人提供**）。「有没有变成空白」这一条已经可测并已测：没有样本宽度为 0、拉丁 pangram 宽度与文本相称、五个组合符号叠在一个字母上的那条**正确地很窄**（组合成功才会窄）。
  4. `contrast-and-zoom.md`——**只剩「看着还能不能读」这一条主观判断**：对比度比值是宿主测试，200% 下 B1 五旅程与 400%（320 CSS px）下目录页重排不丢内容都已是浏览器用例（`p2-zoom`、`p2-reflow`）。
