...
- 最近更新：2026-09-10。M1–M4 已关闭；M5–M8 的功能与自动化验证已交付，等四份人工记录。**新增一个未解决的缺陷：两小时耐久失败**——线性内存最后四分之一没有走平（两分钟的曲线是平的），详见 `docs/validation/p1/m8-baselines.md` 的「两小时」一节。
- 当前进度：4/8 个里程碑完成。自动化只剩一处未过：两小时耐久的内存断言。其余全部是只能由人做的四份记录。
...
- 下一步：① 干净重跑两小时耐久拿到曲线（第一次跑的曲线被我自己并发启动的另一次 Playwright 清掉了结果目录，连报告一起没了），判断是泄漏还是更慢的平台期；② 跑 `cargo xtask verify --suite p1` 作为收尾证据；③ 四份人工记录随时可做。
- 当前阻塞：四份人工记录都需要用户（真实拼音、VoiceOver、对比度与缩放走查、Safari 观察）。另有一个自查缺陷待定：两小时耐久的内存趋势。
