...
- 最近更新：2026-09-10。M1–M4 已关闭；M5–M8 的功能与自动化验证已交付，等四份人工记录。**两小时耐久查出并修掉一个真泄漏**：区域从不回收脚本堆，两小时涨 145 MB（37→189 MB，成倍数跳）；修法是区域在两次泵之间按「攒够 20,000 条垃圾就扫一次」回收，15 分钟复测 36.9→43.5 MB 且尾部持平。详见 `docs/validation/p1/m8-baselines.md`。
- 当前进度：4/8 个里程碑完成。自动化没有已知缺口（两小时复测在跑）；其余全部是只能由人做的四份记录。
...
- 下一步：① 两小时复测跑完把曲线写进 M8 报告；② 跑 `cargo xtask verify --suite p1` 作为收尾证据；③ 四份人工记录随时可做。
