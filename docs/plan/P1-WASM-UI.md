...
- 最近更新：2026-09-08，M2 已关闭，**M3 实施中**（最近两轮为评审意见的核实与修正）；下一会话从本节的「M3 现状」继续。
- 当前进度：2/8 个里程碑完成。
- 代码基线：`3f8e5c3`（M1 首轮）→ `6bb15a1`（M1 收尾并关闭）→ `8427bff`（M2 关闭）→ `d086339`…`6d02836`（M3 的五个提交）→ `6d2017e`（首轮评审修正）→ 本轮复审修正提交（`git log -1`）。工作区 clean。
- 已通过的验证（2026-09-08 本机）：`cargo xtask doctor` 8/8；`cargo test --workspace --lib` 16（registry 3、scheduler 5、binding 8）；`cargo test -p xtask` 10；`cd makepad && cargo test` 13（cargo-makepad 10、wasm_bridge 3）；`cargo clippy --workspace --all-targets -- -D warnings` 无警告；`cargo fmt --all -- --check` 通过；`cargo xtask build-web --example fusion-basic --release` 通过；`cargo xtask build-web --example property-workbench --release` 通过；`npx playwright test m1-probes` **8/8**；`npx playwright test m2-runtime` **11/11**；`npx playwright test m3-state` **2/2**；`npx playwright test --project=property-workbench` **17/17**。
- M2 退出条件对照：全部满足，逐条证据见 `docs/validation/p1/m2-runtime.md`。V1 的正常/缺失/重复容器与 20 轮关闭再挂载 ✔；2 挂载×2 区域关闭一个其余可操作（DOM 与两个 GPU 区域都验）✔；旧句柄与迟到消息无回调 ✔；100 轮以上资源与宿主操作无功能泄漏（区域/计时器/动画帧回到基线；线性内存在 150 轮预热后再跑 100 轮完全不变）✔。
...
- 基线数字（无预算承诺，替换 M1 的对应项）：空闲 4 区域 3 s 内 0 次泵（此前 16 ms 轮询下约 1 次/s）、0 帧；两区域作用域反复挂载/卸载的线性内存有一次 125,173,760 B → 142,016,512 B 的跳变，之后到第 260 轮每 10 轮取样都相同（分配器高水位，不是每轮泄漏）；跳变落在第几轮随运行变化（观察到第 10 轮前，也观察到第 50–250 轮之间），所以泄漏样本取尾部 100 轮；100 轮后浏览器侧资源回到 2 区域/0 计时器/0 动画帧/2 canvas/0 错误。release 产物：wasm 7,716,985 B、JS 163,025 B、CSS 885 B、字体 51,187,844 B。
- **M3 现状**：调度与投递骨架完成，第二份示例已跑通主路径。
  - 已做（调度）：`crates/rustify-ui/src/scheduler.rs`（按到达顺序编号、队列上限 1,024、每批 64、满则返回 Backpressure 且不占用序号、连续动作独立存放只留最新且保持相对位置，5 个单测）；`crates/rustify-ui/src/binding.rs`（每个挂载作用域一个 `ActionSink`，由 `mount` 通过 context 提供；回调内提交的动作进入下一批而不是就地递归；批与批之间用 `setTimeout(0)` 把执行机会还给浏览器，第一批仍同步跑完，4 个单测）；`create_region` 的回调改为一次收下整批动作（`Fn(Vec<A::Action>)`），使批处理真正成立。
  - 已做（第二份示例）：`examples/property-workbench`——1,000 个固定 ID 对象、DOM 属性面板（重命名/调色/删除/上一项下一项）、GPU 选择视图。GPU 侧是本仓自己的 Widget `ObjectGrid`（`examples/property-workbench/src/object_grid.rs`）：一次把全部 1,000 个对象各画一个色块、给当前选中项画白色描边、点击命中回报被选中的业务 ID；网格自己不存业务数据，每次绘制前由应用投影进去。选择的推进规则只有一份，DOM 与 GPU 两侧都走它；页面导出 `snapshot()` 供测试与独立预期比对。浏览器用例 `tests/browser/m3-workbench.spec.ts` 6 项通过，`tests/browser/m3-state.spec.ts` 2 项通过。Playwright 改为两个 project（fusion-basic 4173、property-workbench 4174），各自的静态服务由 `webServer` 数组启动；CI 两个示例都构建。
  - 本轮发现并修掉的三件事：**第一次 props 投影会被丢掉**——`process_to_wasm_with` 原先在派发这一批消息之前跑宿主排队的工作，而区域的第一批消息正是创建应用的 `ToWasmInit`，于是首次投影落在「应用还不存在」的时刻被静默丢弃，区域一直画自己的默认值直到下一次状态变化；改为在派发之后、绘制之前跑（`makepad/platform/src/os/web/web.rs`），并由「无需任何交互，网格就在屏幕上」这条用例守住。`script_apply_eval!` 里嵌套属性必须用 `+:` 合并，用 `:` 会把 `draw_bg` 整个替换成普通对象并被拒绝（报 `expected DrawQuad, got object`）。示例里的 `style="background:#..."` 内联样式违反发布 CSP 的 `style-src 'self'`，调色板改成样式表里的类。
  - 已做（V2 的身份与批量部分）：`rustify_ui::duplicate_key` 找出第一个重复的 key（2 个单测）；`ObjectGrid::set_cells` 遇到重复 ID 就拒绝整份投影、保留上一份无歧义的列表，并把冲突的 ID 通过 `SelectionAction::RejectedDuplicate` 交回应用，应用把它显示成一条 alert。示例新增「recolour 100 / reverse 100 / add 10 / remove 10」四个批量操作，各自是一次受控模型更新；页面 `snapshot()` 增加 `first_colors` 与 `first_ids` 供独立预期比对；`workbench_inject_duplicate_id()` 是专门的测试缝，故意破坏应用自己的 ID 不变式，用来真的走一遍拒绝路径。`tests/browser/m3-workbench.spec.ts` 10 项通过：100 项批量一次更新后 GPU 显示新值、反序 100 项后选择跟着对象走（位置从 2 变成 99、ID 仍是 2）、增删各 10 项后其余 ID 不动、重复 ID 被拒绝且画面不变。
  - 已做（V2 的规模项）：页内向区域 canvas 派发 10,000 次 pointerdown/up，应用侧计数 `accepted` 恰好 10,000、一次不丢一次不重，且顺序正确（每次前进一个对象，1,000 个的环正好回到起点）；整轮 17.9 s。计数是必要的：只看选择位置会被环绕掩盖（1,000 次点击后位置回到 1，看起来像「一次都没生效」）。事件由页内派发而不是走测试夹具，否则测的是夹具而不是运行时。
  - 已做（首轮评审的七处修正）：**回调内关闭区域会打死整个 runtime**——`on_action` 同步 dispose 时，Rust 侧把 Cx 放回注册表后立刻调用 JS `destroy()`，而外层 `wasm_process_msg` 随后还要 `update_array_buffer_refs()`，读到 null 抛异常被升级为 fatal，同 runtime 的其他挂载一起被销毁；改为由 JS 侧持有在途生命周期（`EmbeddedRegion` 记录泵深度，`destroy_region` 在泵仍在栈上时只登记，泵返回后才真正 `destroy()`）。**正常销毁泄漏一条待发消息**——每条 `ToWasmMsg` 在 wasm 里申请 1,024 个 u64，`destroy()` 原先只把 `this.to_wasm` 置空；改为销毁时先 `wasm_msg_free`，模块已 trap 时跳过（`makepad/platform/src/os/web/web.js`）。**卸载后仍投递排队动作**——每个区域一个 `Binding`，`on_cleanup` 关闭它，排队项执行前检查（`crates/rustify-ui/src/binding.rs`，2 个单测：100 条动作在首批 64 条之后关闭，其余 36 条不再进应用）。**入队被拒没有出口**——`GpuRegion` 增加可选 `refused` 信号，`submit_all` 把被拒条数交回应用；示例显示「未执行/可重试」并把 `refused` 写进 `snapshot()`，10,000 次动作用例断言它为 0。**异步启动恢复后不检查存活**——`load_deps()` 在 `await` 之后、碰任何资源之前检查 `destroyed`，构造函数补上 `load_deps()` 的 rejection 出口（修前该用例报 `TypeError: Cannot read properties of null (reading 'ToWasmInit')`）。**fatal 后 DOM 控件仍能调用 wasm**——两份示例的 fatal 处理器改用纯 JS 路径清空挂载容器，不调用 wasm 析构（Leptos 的事件委托监听本身在 wasm 里，JS 摘不掉，但挂载节点移除后它没有可命中的目标）。**快照里的名字不是 JSON**——名字来自输入框，`a"b\c` 会让 `JSON.parse` 抛 SyntaxError，改用真正的字符串序列化。新增 4 项浏览器用例（`m3-workbench.spec.ts` 共 15 项通过），其中三项在修前确实为红。
  - 已做（复审的两处修正）：**Cx 也要活到批次被消费之后**——上一轮只延迟了 JS 侧 `destroy()`，Rust 仍在 `deliver()` 里立刻释放 Cx；而这一批返回的 `FromWasmDrawCall` 用 `WasmPtrF32` 直接指向 Cx 拥有的 uniform 切片，JS 要到 `dispatch_on_app` 才按指针 `new Float32Array(memory.buffer, ptr, len)` 读它。改为释放顺序整个交给宿主：`destroy_region` 只登记待销毁，宿主在自己的泵退出、批次绘制并释放之后，才回调新导出 `rustify_region_release` 丢掉 Cx；`rustify_region_process` 同时拒绝已登记销毁的区域的新消息。用例把区域的 `new_from_wasm` 包起来记录当时的 `live_regions()`，修前最后一批记到 0。**fatal 之后仍有排队任务回到 wasm**——`next_turn` 的续跑定时器由 `window.setTimeout` 改为经宿主 `defer()` 注册，宿主持有 timer id 并在 `enter_fatal` 里全部 `clearTimeout`（任务回调也套上 fatal 出口）；`stats()` 增加 `tasks` 计数，M2 的 100 轮基线一并断言它回到 0。注意可达性：现在每个浏览器事件各自泵一次，一次泵至多产出一个动作，所以两份示例都攒不出「一批 64 条之后还剩 36 条」——用例验的是宿主这一层的取消，端到端那一幕要等某个区域应用一次事件发出多条动作（M4 的拖拽是第一个）。
  - 未做：V2 的 120 Hz 移动夹杂 20 次点击。`Pace::Continuous` 已实现并有单测，但还没有真实生产者，拖拽会是第一个；这一项要等 M4 的几何/指针工作。
