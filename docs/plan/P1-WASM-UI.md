...
> **计划状态：Closed**（用户 2026-09-10 判定一期完成并要求后续只做二期。四份人工记录（真实拼音、VoiceOver、对比度与缩放走查、Safari 观察）始终未产出，按用户决定不再作为门；本计划不再作为工作入口，实施进度见 `P2-WASM-UI.md`）。
>
...
- 最近更新：2026-09-10。**本计划已按用户决定封板，不要再从这里开工**——后续工作全部在 `docs/plan/P2-WASM-UI.md`。
- 当前进度：8/8（用户判定）。自动化部分确已全部交付并通过；**四份只能由人做的记录从未产出**，用户 2026-09-10 决定不再以它们为门。这一行记录的是决定，不是通过的证据。
...
- 已知环境事实：headless 跑在 SwiftShader 上；Playwright 进程测到的耗时比页内测量大 10–30 倍，且与 cargo 抢 CPU 时还会再涨一倍；按 project 分开跑更稳；重新构建产物前先确认没有用例在跑。断言应用状态用页面导出的 snapshot 而不是 DOM 文本。**同一时刻只能有一次 playwright 在跑**：它会启动全部三（现在四）个 webServer，端口被占就直接报错退出；更糟的是新一次启动会清空 `test-results/`，把正在跑的那次的报告和 trace 一起删掉——两小时耐久的第一份曲线就是这么丢的。跑长测前先确认没有别的 playwright 进程，跑完再开下一个。
- **计划偏离（2026-09-09，已记录）**：本计划的「实施者定位」要求不同时开两个未完成的里程碑。M5 与 M6 都只欠 M5 的两项人工验证，短期内无人可做，而用户给出的目标是「按 P2 计划完成剩余开发」，P2 的恢复快照又把「先把 P1 推到 8/8」写成解除 A-2 的唯一办法。因此在 M5/M6 保持未关闭的前提下开始 M7。约束：M7 的任何成果都不得用来关闭 M5 或 M6；这两个里程碑的状态只由那两项人工验证改变。
- **M7 现状**：进行中，已交付并通过的部分见 `docs/validation/p1/m7-deployment-and-recovery.md`。
  - 已做（上下文丢失与恢复）：`RegionState::Lost` 是一个「会回来」的状态而不是失败；丢失时区域被拆掉（GL 对象已被浏览器收回，再画只会画错），浏览器把上下文还回来（`webglcontextrestored`）时重建并在绘制前投影当前 props。20 轮丢失/恢复、每轮之间改一次值，值一次没丢，重建后的区域仍能被真实指针命中。**踩过的坑**：第一版换掉 canvas 元素而不是等恢复，Leptos 复用了同一个元素，新区域拿着已死的上下文编译着色器，区域白屏——比「明显丢失」更糟，因为它看起来像成功了。
  - 已做（有界诊断）：`crates/rustify-ui/src/diagnostics.rs` 登记计划点名的 10 类错误，每类都带下一步；环同时受条数（1,000）与字节（4 MiB）约束，触顶丢最旧的并计数，报告开头必写 `dropped`，截断永远不会被当成全部。`detail` 是 `&'static str`、可变部分一律走类型化字段——这是 NFR-4「不把编辑正文写日志」的结构性保证，而不是靠评审。已接上的产生者：InvalidContainer、OccupiedContainer、GpuInitFailed、GpuContextLost、Backpressure。
  - 已做（失败与截断的两个边界）：区域完全起不来时「2 秒内提示」在页内测量（穿过测试进程的一次往返就比被测的预算还贵），并验证同一页的 DOM 半边照常工作；诊断环的上限在浏览器里也证明了——1,100 次被拒的挂载，留下 1,000 条，其余计入 `dropped`。
  - 已做（部署）：同一份产物在 `/tools/demo/` 子路径下启动、绘制，且不动宿主页面；请求全部同源且在 base 之下；策略里没有 `unsafe-inline`/`unsafe-eval`。`cargo xtask serve --fault` 把三种真实的部署故障注入到服务端（文件没拷过去、拷坏了、两个 build 的资产混装），测的是运行中的产物而不是改过的副本；错版桥在启动时被拒并说明要重新部署匹配的资产；字体缺失/损坏/截断只损失字形，运行时照常启动、两半照常同步、没有 trap。`cargo xtask report-size` 从产物目录读出六类字节数。新增第三个 Playwright project（deployment）。
  - 已做（能力拒绝）：嵌入契约拒绝的五件事（开链接、改 URL、走历史、全屏、文档标题）现在按「每个区域每类一次」进环，带原因、下一步和区域号。接线时发现一件一直在悄悄发生的事：**Makepad 建窗口时会给窗口命名，所以每个区域启动时都会请求设置文档标题并被拒**——拒绝本身没错（嵌入的区域没有文档标题可设），但在此之前没人看得见它。它现在是每个 runtime 记录里的第一条。
  - 已做（资产失败与共享 trap）：`AssetLoadFailed` 有了产生者。分叉的 `platform/src/script/res.rs` 的 `handle_http_error` 本来只打一行日志，嵌入方无从知道自己部署的文件没到；现在它调用一个可选钩子（由 `identify_runtime` 装上），记录路径和下一步。**但三类资产失败只有一类会被报告**：404 让加载失败，能报；损坏和截断都带着 200 到达，加载是成功的，失败发生在解码里（分叉的字体代码，比钩子更深），不报。三类都验证了「只丢字形、不丢应用」，用例明确断言哪一类可见、哪两类不可见，不含糊过去。共享 trap 的影响范围也有了自动检查：两个作用域四个区域，trap 后全部消失，提示说明丢了什么、该做什么，页面上再也拿不到应用入口。
...
- **M8 现状**：自动化部分已全部完成，见 `docs/validation/p1/m8-baselines.md`。
  ...
  - 已做（两小时耐久，V12）：**跑了，失败了，查出并修掉一个真泄漏**。72,000 次动作全部到达（accepted==sent、refused 0、10.00/s、errors 0），但线性内存 36,896,768→189,136,896（145 MB），且是成倍数跳（+2.4/+4.1/+8.1/+16.1/+32.1/+64.1 MB，间隔也在翻倍）——一个容器按动作固定增长、按二倍扩容的签名。容器是脚本 VM 的堆：每次给 shader 设值的 props 应用都会走一次 `script_apply_eval!`，每次求值留下 3 个对象，而**嵌入区域从来没有回收过脚本堆**（桌面应用的脚本只在启动时跑一次，收集器只有脚本自己够得着）。分叉内 10 毫秒的宿主单测复现了同一件事（500 次求值多出 1,500 条活对象，一次 sweep 全收回）——`what_a_reused_eval_callsite_leaves_behind_is_collectable`，它钉住的是「那些对象是垃圾不是状态」这条修复所依赖的性质。修法在区域的泵之间（`crates/rustify-makepad/src/wasm/host.rs`）：本区域自上次清扫以来攒够 20,000 条垃圾就扫一次——是「量」不是「周期」，闲着的区域一次都不扫。15 分钟复测：9,000 发 9,000 收 0 拒，内存 36,896,768→43,515,904 且最后 28 个采样持平（旧产物在同一时刻已经 71 MB 还在翻倍）。**两小时复测本会话没跑完**，见「下一步」。顺带修掉：分叉 workspace 排除了 `platform/`，它的脚本 VM 单测从来没被跑过，现在 `verify --suite p1` 与 CI 都点名跑。
- 未竟事项（记录在案，不再阻塞任何人）：① 四份人工记录；② 脚本堆泄漏修复后的**两小时**复测没跑完（15 分钟复测通过：9,000 发 9,000 收 0 拒、36.9→43.5 MB 尾部持平），命令是 `RUSTIFY_ENDURANCE_MINUTES=120 npx playwright test --project=property-workbench m8-endurance.spec.ts`，曲线位置 `docs/validation/p1/m8-baselines.md` 的 `<!-- endurance-two-hours-after-fix -->`；③ 本会话改过 xtask 与示例后，`cargo xtask verify --suite p1` 未在当前产物上重跑。以上三项若日后要补，按此处执行即可。
- 当前阻塞：无。本计划已封板。
