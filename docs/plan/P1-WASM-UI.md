...
- **M3 现状**：调度与投递骨架完成，第二份示例已跑通主路径。
  ...
  - 已做（第二份示例）：`examples/property-workbench`——1,000 个固定 ID 对象、DOM 属性面板（重命名/调色/删除/上一项下一项）、GPU 选择视图（色块、名称、序号、上一项下一项按钮）；选择的推进规则只有一份，DOM 与 GPU 两侧都走它；页面导出 `snapshot()` 供测试与独立预期比对。浏览器用例 `tests/browser/m3-workbench.spec.ts` 5 项通过，`tests/browser/m3-state.spec.ts` 2 项通过。Playwright 改为两个 project（fusion-basic 4173、property-workbench 4174），各自的静态服务由 `webServer` 数组启动；CI 两个示例都构建。
  - 本轮发现并修掉的两件事：`script_apply_eval!` 里嵌套属性必须用 `+:` 合并，用 `:` 会把 `draw_bg` 整个替换成普通对象并被拒绝（报 `expected DrawQuad, got object`）；示例里的 `style="background:#..."` 内联样式违反发布 CSP 的 `style-src 'self'`，调色板改成样式表里的类。
  - 未做：稳定 key 与重复 key 的开发期报告（§5.3）；GPU 侧一次绘制 1,000 个可选对象的自定义 Widget（当前 GPU 视图只画当前选中项）；V2 的完整规模（10,000 个带序号动作、100 项批量、120 Hz 移动夹杂 20 次点击、反序 100 项与增删各 10 项、重复 ID）。`Pace::Continuous` 已实现并有单测，但还没有真实生产者，拖拽会是第一个。
