...
- **M3 现状**：调度与投递骨架完成，第二份示例已跑通主路径。
  ...
  - 未做：稳定 key 与重复 key 的开发期报告（§5.3）；V2 的完整规模（10,000 个带序号动作、100 项批量、120 Hz 移动夹杂 20 次点击、反序 100 项与增删各 10 项、重复 ID）。`Pace::Continuous` 已实现并有单测，但还没有真实生产者，拖拽会是第一个。
