...
- 最近更新：2026-09-13（**M8 完成，P3 收尾 8/8**；A-4 由用户豁免，记为未测）
- 当前进度：**8/8 个里程碑完成 · P3 Closed**
- 当前状态：**M1–M8 全部完成并跑绿，P3 收尾。**A-4 于 2026-09-13 由用户豁免——用户在被明确问及「跑一遍」还是「豁免」时答「豁免」，七个步骤未执行，记为「未测·用户豁免」，不记通过；`verify --suite p3` 因此新增 `Waived` 记录状态（原先豁免会被渲染成 `held`，即「有人做了并写下来」，对豁免不实），现在逐次打印「untested, waived by the user - not a pass」并以「P3 is complete with 1 waived record(s): those checks were not performed and are not claimed.」收尾。**本期不宣称 B2/B3 的屏幕阅读器支持**；自动化证明的是可访问性树正确（行号是十万行里的真实行号、一个网格一个 tab 停、查询入口可达、键盘可改），未证明的是念出来之后是否可导航。此豁免沿用 P2 先例（2026-09-11），不扩大 §0.6 已排除的范围（WCAG 2.2 AA 整体评审与完整 AT 矩阵仍属 P4）
- 最近完成：**M8 · 门、报告与交接**（2026-09-13）
- 下一步：**P3 已 Closed，本计划不再有下一步。**后续工作属 P4，范围见 §0.6 与 `docs/reports/p3/known-limitations.md`：Windows/Linux/移动矩阵与 NVDA、WCAG 2.2 AA 整体评审（含本期豁免的 VoiceOver 旅程）、两份迁移示例、5 名新用户研究、稳定弃用窗口、局部热更新、多线程/共享内存增强模式、GPU 自绘文本编辑器与 GPU 表格、区域级 trap 隔离、对外发布。开新计划前先读 `docs/reports/p3/known-limitations.md` 的「Measured, and unresolved」三条：B1 时延 45.5 ms 只剩 4.5 ms 余量且漂移无定论、增长类断言对机器状态敏感、工作集预热轮数随构建变化
- 当前阻塞：无。P3 全部退出条件已满足（22 个自动化步骤 0 失败 + 1 份用户豁免记录）
- 代码基线：`6355a0b`（M8 文档收口；M8 实现为 `7d4c71d`，M7 为 `12a9d5e`，M6 为 `bb76c29`，M5 为 `aa0b1e9`，M4 为 `956b89d`，M3 为 `d0a193e`，M2 为 `122a49f`，M1 为 `a6968ca`）
...
4. 键盘：方向键、PageUp/PageDown（一屏）、Home/End（行首尾）、Ctrl+Home/Ctrl+End（首/末行）、Enter/F2（编辑当前行详情）、空格（切换选择）。焦点移到窗口外时，键处理器只记新位置并按需滚动纵横两轴，落焦点由跟随可见范围的效果完成（同一帧内）：槽位显示哪一行是响应式属性，按行号找单元格会找到上一刻的那个；按槽位找到的才是即将带上该行号的元素。表格自己放的焦点不回读——`focusin` 读的正是那批还没写的属性。
