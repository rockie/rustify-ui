...
- 最近更新：2026-09-12（M4 完成并全部跑绿）
- 当前进度：4/8 个里程碑完成
- 当前状态：M1–M4 均已完成并全部跑绿（见完成记录）。M4 交付场景的真实输入路径（平移、框选、点选、hover）、`scene_layout::{within,pick}` 与其 TypeScript 孪生、`/scene` 视图、接受计数出口，并**解除 F24**：props pump 在微任务里要的是下一帧，而上一帧回调未跑时的重复请求会被丢弃，于是两次请求落在同一帧、第三帧空过；`embedded.js` 的 `present()` 就地兑现那一帧，驱动与呈现从 0.5:1 回到 1:1。连带把画布在 flex 行里长到 1,964 px（比视口还宽）也修了，两件事一起把探针②的帧间隔砍掉近一半：无头 SwiftShader p95 50.10 → **33.40 ms**、有头 Chrome 17.60 → **9.30 ms**，两边都是 1:1；A-3 的分支不变（无头仍超 20 ms 预算，帧门仍只在有头跑）。代价记在 `docs/validation/p3/m4.md`：B1 时延 p95 41.7 → **44.9 ms**，门 50 ms 仍过，余量剩 5 ms，M8 是检验点。**正文已随实现修订五处**：§2 的 `SceneRegion` 行（没有 `layout` prop、新增 `labels`、修饰键随动作走、三种手势的判定）、§1.2 的 `embedded.js` 行（`present()`）、§5.4 第 3 条（`within`、`additive`、3 px 判定）、§11 的 F24 行（已解除）、§12 增五条教训
- 最近完成：**M4 · 场景**（2026-09-12）
- 下一步：**M5 · 实例隔离与有界重试**——前置已在 M1 交付（loader 的按实例 `boot`、导出调用边界、按 glue URL 归属的页面级 `error`、每实例 `AbortController`、上限 3 次的 `restart()`、`release_container()`，以及 fusion-basic 的两个真实 trap 出口；探针③已证明 ADR-8 选项 B 与 D15 都成立，不走失败分支）。本里程碑把这些从探针版本变成产物：`host.rs` 暴露 `listener_options()`，`router.rs`/`overlay.rs` 的页面级监听带实例 `signal` 注册、URL 所有者带实例号，`region.rs` 的三次 GPU 启动重试与 `GpuInitRetry`，fusion-basic 双实例模式，四个示例的 `app.js` 把 `runtime_fatal` 改成实例级，并写 `docs/architecture.md` 的实例模型与 `docs/compatibility.md` 的实例内存代价与重启上限。退出条件见 §10 的 M5 行与 §9.1 的 V5（四种 trap 入口都要走到同一个提示；另一实例用 100 次动作的接受数判健康，不用 `pageerror = 0`）
- 当前阻塞：无。A-4（VoiceOver 人工记录或用户豁免）仍开放，最晚 M8 确认
- 代码基线：`956b89d`（M4 的提交；M3 为 `d0a193e`，M2 为 `122a49f`，M1 为 `a6968ca`）
