...
- 最近更新：2026-09-12（M5 完成并全部跑绿）
- 当前进度：5/8 个里程碑完成
- 当前状态：M1–M5 均已完成并全部跑绿（见完成记录）。M5 交付实例边界：一页两个应用实例，各有自己的线性内存、宿主 hooks 与诊断环；trap 的四扇门（导出调用、宿主任务、DOM 处理器、页内模拟）都只带走本实例并走到同一个提示，另一实例 100/100 接受动作。`Drop` 在 trap 后不运行，所以页面级的东西由 JS 侧持有：`window`/`document`/容器上的监听一律带实例 `signal` 注册，地址栏所有者是文档根上带实例号的标记，宿主按固定顺序释放（abort → 清标记 → 作废任务与计时器 → 销毁区域 → 通知页面），重启后新实例重新成为所有者且 `popstate` 监听数回到基线，第 4 次失败只剩重载。`GpuRegion` 改为 0/250/750 ms 三次询问，前两次记 `GpuInitRetry`（Info），诊断目录因此 17 类。**正文已随实现修订八处**：§0.5（`AbortController` 归宿主 runtime，不归 loader）、§2 的 loader 行（controller 与 `enter_fatal` 同在 `embedded.js`，经 `hooks.signal` 暴露）、§2 的 `GpuRegion` 重试行（两次等待走新增的 `host::defer_after`）、§3 的实例行、D15、§5.5 第 1 条与第 3 条（`enter_fatal` 在宿主；`UrlClaim` 析构只清仍属于自己的标记）、§9.1 的 V5（GPU 重试断言复用 `m7-recovery.spec.ts` 的注入夹具）、§12 增五条教训
- 最近完成：**M5 · 实例隔离与有界重试**（2026-09-12）
- 下一步：**M6 · B0、B4、空闲与资源**——把 fusion-basic 的默认挂载改成 B0（§6.3：DOM 表单 10 项 + 一个区域内 20 个 GPU 控件 + 一份共享状态，只用一期组件，见 D9），并逐条列出因此要改的既有断言（M5 之前不许动，就是为了在这一处集中改）；B4 夹具按 §5.7.3 = 两个长期存活实例各挂一个作用域（各两区域）、只挂卸不重建实例（`boot_instance` 已在 M5 交付）；`web_gl.js` 的 `gpu_bytes` 记账（`frames` 已在 M1 落地，D8）与 `sources.lock.json`／`docs/compatibility.md` 的账本口径登记；新增 `tests/browser/p3-{b0,idle,memory}.spec.ts`。退出条件见 §10 的 M6 行与 §9.1 的 V6（空闲 60 s `frames` ≤ 1、CPU ≤ 1 个百分点；两种隐藏各 60 s、1 s 内停、500 ms 内恢复；100 轮隐藏/恢复重复 0；B0/B2 内存 ≤ 128/384 MiB、GPU ≤ 128/256 MiB；B4 预热 20 轮后 80 轮增长 ≤ 8 MiB 且后 40 轮 ≤ 64 KiB/轮；卸载残留 0）；不可测项按未测记并写明
- 当前阻塞：无。A-4（VoiceOver 人工记录或用户豁免）仍开放，最晚 M8 确认
- 代码基线：`aa0b1e9`（M5 的提交；M4 为 `956b89d`，M3 为 `d0a193e`，M2 为 `122a49f`，M1 为 `a6968ca`）
