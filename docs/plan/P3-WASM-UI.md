...
> **计划状态：Closed**（2026-09-13 本期验收收尾：8/8 里程碑完成，`cargo xtask verify --suite p3` 22 个自动化步骤 0 失败——九个浏览器 project 全过、四示例双干净构建逐字节一致、七份报告齐备；B0/B2/B3 的预算全部成为断言并各留余量，两个帧门的负向探针都如预期必败；B3 帧门按 A-3 只在有头 Chrome 跑，CI 与本套件都不含它，结果由人工运行记录在 `docs/reports/p3/performance.md`。A-4（VoiceOver 人工走查）由用户明确豁免，七步未执行，记为「未测·用户豁免」不记通过——**本期不宣称 B2/B3 的屏幕阅读器支持**。未测项另有 `document.hidden`（本无头浏览器无法置真）。本期完成不等于完整 PRD 达标：Windows/Linux/移动矩阵、WCAG 2.2 AA 整体评审、迁移示例、新用户研究、弃用窗口与对外发布均属 P4，清单见 §0.6 与 `docs/reports/p3/known-limitations.md`。`P1-WASM-UI.md`、`P2-WASM-UI.md` 继续保持封板）。
>
> **完成后评审更新（2026-09-14）**：12 条意见均经核实并修复，新增 21 个浏览器回归用例。功能回归与 B2 性能门通过；本轮验证范围及未重跑项见 [评审修复记录](../validation/p3/review-fixes.md)，原 M8 全套结果保留为历史证据。
>
...
- 最近更新：2026-09-14（完成后评审修复与验证收口）
- 当前进度：**8/8 个里程碑完成 · P3 Closed；本轮评审修复已验证**
- 当前状态：12 条评审意见均成立并已修复，新增 21 个浏览器回归用例；稳定 ID 删除/编辑、筛选与排序提交、表格焦点/尺寸/ARIA、四示例重启行为已验证。67 项浏览器回归、8 项表格复测（新增表尾尺寸焦点 1 项）、B2 性能门 4 项、243 项 Rust 测试、原生与 wasm Clippy、格式与来源/CSS 检查通过；§5.1–§5.3、§5.5 与 `docs/data.md` 已同步。本轮未重跑完整 P3 套件，历史 M8 全绿记录不冒充本工作树的全量验证，详见 `docs/validation/p3/review-fixes.md`。
- 最近完成：**M8 · 门、报告与交接**（2026-09-13；完成后评审修复于 2026-09-14 验证完成）
- 下一步：本轮评审修复无待办；后续范围见 §0.6 与 `docs/reports/p3/known-limitations.md`。
- 当前阻塞：无；A-4 VoiceOver 仍为「未测·用户豁免」，不宣称屏幕阅读器支持。
- 代码基线：`dirty@845ed96`（本轮关键改动：`crates/rustify-components/src/data_table/view.rs`、`crates/rustify-makepad/src/wasm/host.rs`、`crates/rustify-makepad/web/embedded.js`、`examples/data-workbench/src/`、四示例 `app.js`、`web/loader.js`、`tests/browser/p3-{table,jobs,restart}.spec.ts`）
...
| M4 | 2026-09-12 | 场景，以及它被画进哪一帧。`scene_layout::{within,pick}`（框选取相交、命中取上层，只算面积够得着的行列而不是一万个矩形）与 `dataset.ts` 的孪生（孪生用全扫描——跟着优化一起写的第二实现会跟着一起错）；`scene_view.rs` 的真实输入路径：空白处按下拖动是平移、矩形上按下拖动是框选、按下到释放位移 ≤ 3 px 是点选、滚轮是平移、hover 是流；`scene_region.rs` 的 `Camera`/`Marquee`/`Pick`/`Hover`/`Viewport`，修饰键随动作走（并入还是替换是按下那一刻的事实），回流值等于自己发出的值就不覆盖本地相机；`/scene` 视图（查找入口、选中列表 50 项 + 计数、详情表单重命名、清空）与接受计数出口。**F24 解除**：相位查明——props pump 在微任务里要的是下一帧，而上一帧回调未跑时的重复请求被丢弃，于是两次请求落在同一帧、第三帧空过；`embedded.js` 的 `present()` 就地兑现那一帧，页内实测 53 呈现 / 54 驱动（原为 35 / 71）。**跑用例查出三个问题**（详见 `docs/validation/p3/m4.md`）：①画布是替换元素，宿主每帧把实际尺寸写回它的宽高属性，flex 行里沿自己主轴的 `min-width: auto` 就撑到 1,964 px、比 1,440 px 的视口还宽（M2 在纵向撞过同一条规则），两个方向都显式写尺寸并清零 min——场景此前一直在画将近两倍的对象；②表单只能保存一次：`submitting` 只有 `form.submitted()` 能清，同步保存从来没有回报，表格的详情表单从 M2 起就带着这个缺陷，而它的用例只保存过一行；③选中列表随选择变高，把画布往下推，点下去就落在别处（列表写死高度，用例每次手势前重量画布）。另修一处用例自身的错：等差步长取模不是散布——137 对跨度 1,096 只有 8 个余数，而两层相距 130 px，一百个采样点一个上层矩形都没碰到，改用随机数发生器并先断言样本里确有上层矩形。探针②因此在本构建上重测：无头 SwiftShader p95 50.10 → **33.40 ms**（> 50 ms 占比 13.9% → 0%）、有头 Chrome 17.60 → **9.30 ms**，两边都从 0.5:1 回到 1:1；A-3 分支不变 | `docs/validation/p3/m4.md`；七个 project 全绿，逐个单跑：`data-workbench` **43/43**（7 探针 + 11 表格 + 14 作业 + 11 场景）、`fusion-basic` **56/56**、`property-workbench` **129/129**、`component-catalog` **46/46**、`budget` **4/4**（冷 p95 178 ms、热 172 ms、首载 2,777,134 B、B1 时延 p95 **44.9 ms**——较 M3 的 41.7 ms 升 3.2 ms，门 50 ms 仍过，代价与退路写在报告里）、`workbench-deep` **6/6**、`deployment` **6/6**；`cargo test --workspace --lib` 202（components 68、makepad 3、ui 131）、`-p data-workbench` 39、`-p xtask` 23；`cd makepad && cargo test` 与脚本 VM 测试通过；clippy `-D warnings` / fmt 通过；`cargo xtask catalog --check`（20 行）、`css --check`、`sources verify` 通过（分叉未改动）；四个示例均以本工作树重新构建后再跑 | `956b89d` |
| M5 | 2026-09-12 | 实例隔离与有界重试。一页两个实例（`boot_instance` 挂夹具），trap 的**四扇门**——导出调用（调用边界先报后重抛）、宿主任务（runtime 自己的 `try`）、DOM 处理器（按 `error.stack` 里的 glue URL 归属）、页内模拟——全部只带走本实例并走到同一个提示（含「未持久化修改已丢失」与重启入口），另一实例 100/100 接受动作、区域仍在、诊断是自己的。`Drop` 在 trap 后不运行，页面级资源改由 JS 侧持有：`host.rs` 的 `listener_options()` + `crates/rustify-ui/src/listeners.rs`，`router.rs`/`overlay.rs` 的 `window`/`document`/容器监听一律带实例 `signal` 注册；地址栏所有者从 `thread_local!` 改为文档根上的 `data-rustify-url-owner="<instance>:<scope>"`，`UrlClaim` 析构只清仍属于自己的那一个；`embedded.js` 的 `enter_fatal` 按固定顺序释放（`abort()` → 清本实例标记 → 清计时器与任务 → 销毁区域 → `on_fatal`）。`region.rs` 改为 0/250/750 ms 三次询问 canvas，前两次记 `GpuInitRetry`（Info，带序号），诊断目录 17 类。**三处与计划不同已回写正文**：`AbortController` 与 `enter_fatal` 同在宿主而不在 loader（§0.5/§2/§3/D15/§5.5）；新增宿主契约 `defer_after`（重试的等待持有 wasm 回调，和 `defer` 同一条理由归宿主，失败即清）；GPU 重试的浏览器侧断言复用 `m7-recovery.spec.ts` 的既有注入夹具（§9.1 V5）。**跑用例查出四个问题**（详见 `docs/validation/p3/m5.md`）：①示例的失败提示写死「只有实例 1 才通知页面」，重启后页面显示的是实例 2、3，第二次失败一声不响——页面改为跟踪自己在显示哪个实例；②比较两实例诊断的用例在 trap 之后去问死实例，拿到 `InstanceDead`（调用边界正在做它该做的事），改为 trap 之前读——反着写的用例会「通过」；③既有卸载残留用例的 150 轮预热不够了：本构建的工作集约第 250 轮才走完（三次复跑逐字节相同，600 轮之后不再增长），预热改 300 并把实测写进注释——**这是对既有断言的修改，原因是 M5 的每次挂载多了点活让分配器落点后移，不是泄漏**；④有界重试让一个区域记三条而不是一条，`m2-runtime.spec.ts` 的 2 改为 6 并说明靠 `kind` 区分。另补两个既有示例缺的 CSP 用例（`p3-policy.spec.ts`，一份文件在两个 project 里跑） | `docs/validation/p3/m5.md`；七个 project 全绿，逐个单跑：`fusion-basic` **65/65**（56 既有 + 9 新）、`property-workbench` **130/130**、`component-catalog` **47/47**、`data-workbench` **43/43**、`budget` **4/4**（冷 p95 184 ms、热 172 ms、首载 2,780,344 B、B1 时延 p95 **45.3 ms**——较 M4 的 44.9 ms 升 0.4 ms，门 50 ms 仍过，余量 4.7 ms）、`workbench-deep` **6/6**、`deployment` **6/6**；`cargo test --workspace --lib` 204（components 68、makepad 3、ui 133）、`-p data-workbench` 39、`-p xtask` 23；`cd makepad && cargo test` 与脚本 VM 测试通过；clippy `-D warnings` / fmt 通过；`cargo xtask catalog --check`（20 行）、`css --check`、`sources verify` 通过（分叉未改动）；四个示例均以本工作树重新构建后再跑 | `aa0b1e9` |
| M6 | 2026-09-12 | B0、B4、空闲与资源。fusion-basic 的默认挂载改为 **B0**：10 个 DOM 控件（文本 ×3、多行 ×1、复选 ×3、滑块 ×2、按钮 ×1，全部一期组件，D9）+ 一个区域内 **20 个 GPU 控件**（十类各两个）+ 一个 `B0State`，页面默认只挂它，几何/路由/trap 夹具各走各的导出；一期用例要的两个计数器作用域由 `support.ts` 的 `waitForReady` 挂回。B4 夹具 = `boot_second_instance()` + 两个长期实例各挂一个两区域作用域的轮次（不重建实例）。`web_gl.js` 按对象记字节（`note_gpu_bytes`：buffer 重分配、四个纹理入口按格式、渲染目标），`embedded.js` 的 `stats()` 汇总为 `gpu_bytes`，区域销毁即归零；示例新增 `live_components()` 用于卸载残留断言。新增 `p3-{b0,idle,memory}.spec.ts`。**跑用例查出四个问题**（详见 `docs/validation/p3/m6.md`）：①标了 `hidden` 的区域根本没挂起——`[hidden]{display:none}` 输给页面自己的 `.b0{display:grid}`，画布尺寸照旧，「隐藏」量到的是可见开销（页面加一条 `!important` 覆盖，教训进 §12）；②页面长过视口后 `page.mouse.click` 打在视口外既不报错也不命中，B0 与 m3-state 两处手势改为先 `scrollIntoViewIfNeeded` 再量；③增长门的堆样本必须在 `HeapProfiler.collectGarbage` 之后取，否则 80 轮拟合出的 49,852 B/轮里大半是锯齿（收集后 10,444 B/轮）；④「作业没在跑」在作业开始前也是真的，B2 峰值用例改为先等 `slices > 0`。**因默认挂载改动的既有用例共十条，逐条列在报告里**：八条页面级计数（区域 4→5 / 2→3、画布 2→3），一条把「页面上有几个 textarea」改成「有几个 `textarea.cx_webgl_textinput`」（B0 自己有一个多行输入，而该用例问的是 Makepad 的隐藏代理），一条不是断言而是重新瞄准（m3-state 每轮重量画布）；另有 `m7-deployment.spec.ts` 的三条字体故障用例改经 `waitForReady` 取得计数器作用域、`p3-probes.spec.ts` 的探针③自挂 `scope-a`。**`document.hidden` 记未测**：另开标签页置前与 CDP `Page.setWebLifecycleState` 都无法在本无头浏览器上把它置真，按 §5.6.2 的既定退路跳过并写明 | `docs/validation/p3/m6.md`；七个 project 全绿，逐个单跑：`fusion-basic` **74/74**（2 skipped：B2 内存属另一 project、`document.hidden` 无法产生）、`data-workbench` **44/44**（2 skipped 同理）、`property-workbench` **130/130**、`component-catalog` **47/47**、`budget` **4/4**（冷 p95 182 ms、热 170 ms、首载 2,780,592 B、B1 时延 p95 **46.3 ms**——较 M5 的 45.3 ms 升 1.0 ms，门 50 ms 仍过，余量 3.7 ms，M8 是检验点）、`workbench-deep` **6/6**、`deployment` **6/6**；页内实测：B0 10/20/1/1 与首屏单字体、峰值 45.0 MiB / 57,576 B，B2 峰值 79.9 MiB / 1,976 B，空闲 60 s 0 帧 0.023%（空白页 0.011%），隐藏 1 s 内停 60 s 不动、恢复 10.5 ms，100 轮 100/100，B4 增长 1.52 MiB / 10,444 B 每轮，卸载残留全 0；`cargo test --workspace --lib` 204、`-p data-workbench` 39、`-p xtask` 23；`cd makepad && cargo test` 与脚本 VM 测试通过；clippy `-D warnings` / fmt 通过；`cargo xtask catalog --check`（20 行）、`css --check`、`sources verify` 通过（`web_gl.js` 作为分叉改动报为 drift，口径登记在 `docs/compatibility.md`）；四个示例均以本工作树重新构建后再跑 | `bb76c29` |
| M7 | 2026-09-13 | 故障矩阵、长时与诊断开销。**只交付测试与报告，不动任何 Rust 与 JavaScript 源码**——四个示例产物与 M6 逐字节相同，所以本行的每个数字都是对 M6 那棵树的复测。`p3-faults.spec.ts` 四类 × 20 次，每次对照一份**跑之前就写好的**预期清单：字体 404（经 `serve --fault missing:`，一次故障一个文档，20 次就是 20 次启动）只该少掉字形、答复反序（两次加载相隔 380 ms、倒序应答）该什么都不花、区域动作内关闭自己的作用域该只花掉那个作用域、`lose_context`/`restore_context` 该只花掉像素。「已确认」一律读页面自己的 snapshot 而不是控件——故障可以花掉一帧、一个上下文或一个答复，但不能花掉其中任何一个值。`p3-endurance.spec.ts` 把 B4 的时长做成参数（默认 2 min 回归，`RUSTIFY_ENDURANCE_MINUTES=120` 为验收），节奏由页内计时器打——每个动作走一趟 harness 等于在设定速率而不是测量它。`p3-diagnostics.spec.ts` 是 R39 AC2 的开/关对照，两遍各 1,000 次跨区动作。**跑用例查出四个问题**（详见 `docs/validation/p3/m7.md`）：①2 min 的耐久量到的是缓存在填——这些区域画的每个标签都带计数、没有一个字符串画过两次，整形器与排版器各 4,096 条缓存要走完才算稳态；10 min 跑出形状（涨 6 min、平 4 min），预热从 2,200 改 5,000，改完 4 min 跑动 **0 字节**（C-6 在一个字符串永不重复的夹具上又应验一次）；②关闭作用域后「新作用域从第一个对象开始」只在第一轮成立——对象活在应用里而不是显示它的作用域里，期望改成「对象数不变，且新作用域能接受一个动作并报出来」，比原来那条更强；③同一页跑两遍、每遍一半动作是重命名，第二遍要的名字第一遍用过，应用正确地拒绝，而驱动看到的是「一个动作没到达」——每遍改为各自命名；④`set_diagnostics` 返回的是被替换掉的旧值，断言返回值会让一个完全照办了的页面判红，改读 `diagnostics().recording`。**另有一处是调度不是缺陷**：本期第一次全项回归紧接在 2 h 之后跑，`m8-baseline.spec.ts:102` 成了 135 条里唯一的红；同一棵树重跑即过，教训（2 h 放最后）已进 §12，用例未改 | `docs/validation/p3/m7.md`；**2 h 验收**：120 min at 10.00/s、**72,000 发 72,000 收**（36,000 + 36,000）、0 丢 0 重 0 错、4 个区域存活，尾部 p95 **1.3 ms** 对头部 1.5 ms（7,200 次采样，最差 19.4 ms），两实例 +5,963,776 B / +5,701,632 B **且第 43 分钟后 77 分钟 0 字节**（每 10 s 一点、共 720 点）；诊断开销 **−0.4%**（p95 45.10 ms 记录 / 45.30 ms 不记录，门 ≤ 5%）；七个 project 全绿，逐个单跑：`property-workbench` **135/135**（37.6m，含四类故障与诊断对照）、`fusion-basic` **75/75**（48.0m，2 skipped 同 M6 口径；2 min 回归 1,200 发 1,200 收、两实例各 0 字节增长）、`data-workbench` **44/44**、`component-catalog` **47/47**、`budget` **4/4**（冷 p95 188 ms、热 159 ms、首载 2,780,592 B、B1 时延 p95 **45.5 ms**——较 M6 的 46.3 ms **回落** 0.8 ms，连涨三期终止，门 50 ms 余量 4.5 ms）、`workbench-deep` **6/6**、`deployment` **6/6**；`cargo test --workspace --lib` 204（components 68、makepad 3、ui 133）、`-p data-workbench` 39、`-p xtask` 23；`cd makepad && cargo test` 13 与脚本 VM 11（2 ignored）通过；clippy `-D warnings` / fmt 通过；`cargo xtask catalog --check`（20 行）、`css --check`、`sources verify` 通过；四个示例均以本工作树重新构建后再跑 | `12a9d5e` |
| M8 | 2026-09-13 | 门、报告与交接。**P3 的每个数字都从测量值变成了能让构建失败的断言。**`budgets.ts` 的 B0/B2/B3 段成为断言，文件头与「不覆盖什么」重写为「哪个门测哪个负载、由哪个 project 断言、在哪跑」；三个门 project（**由计划的两个改为三个**：一个 Playwright project 只有一种 `headless` 设置，而 §7 要求 B2 的门 headless、B3 的门只在有头 Chrome——环境那条有探针②的实测支撑，故按环境拆）：`budget-minimal`（B0 冷/热/首载）与 `budget-data`（B2 帧门 + 排序/筛选/取消 + 孪生比对）无头且进 CI，`budget-scene`（B3 帧门）有头专跑、不进 CI 也不进 `verify --suite p3`。**每个帧门前置 5 s 负向探针并且必须失败**：B3 用应用自己的 `freeze_scene(true)`，B2 不需要开关——表格的内容版本是「滚到哪」的函数，把它驱动回原位画面就真的停了，§9.3 预留的 `freeze_table` 因此未建。`xtask/src/verify.rs` 的 `P3` suite（4 示例 / 9 project / 7 报告 / 1 人工记录）+ 三套件互不相同的单测；`docs/reports/p3/` 七文件 + README；`docs/quickstart.md`（删过期的「没有 router/workspace/大数据视图」、补实例模型一节）、`docs/data.md`（补「它要满足什么门」）、`docs/architecture.md`（「十八类」改二十类、「P2 M4–M6 或更晚」改为已交付/仍未提供）、`docs/components.md`（两处「十八」）、`docs/compatibility.md`（浏览器矩阵改写，列明哪个门在哪跑）收口。**A-4 由用户豁免**（2026-09-13，被明确问及后答「豁免」），七步未执行，记为「未测·用户豁免」不记通过；为此给 `verify` 新增 `Waived` 记录状态——原先豁免会被渲染成 `held`（「有人做了并写下来」），对豁免不实。**跑用例查出两个真 bug**：①`verify.rs` 的步骤名用「固定列表 + `_ =>` 兜底」生成，第四个示例被报成 `property-workbench`、三个新 project 会全被报成 `deployment`——构建与测试都对，只有报告在撒谎，改为从实际名字生成；②文档里四处陈述已不再为真（类别数、里程碑相对的「稍后交付」、xtask 子命令清单、P1 时期的浏览器矩阵），不影响行为，只影响读者的结论 **2026-09-14 完成后评审修复**：12 条问题均修复，新增 21 个浏览器回归用例；尺寸观察器在卸载与 fatal 时均释放。 | `docs/validation/p3/m8.md`；**`cargo xtask verify --suite p3`：22 个自动化步骤 0 失败**，九个浏览器 project 全过、四示例双干净构建逐字节一致（`d5caecb51f6ad40d` / `3e70d1727470f816` / `457e9c5ac84ecc81` / `5d7c7fe50f0fd9a2`）、七份报告齐备，结语为「P3 is complete with 1 waived record(s): those checks were not performed and are not claimed.」；**门的实测**：B0 冷 p95 **149 ms**（门 2,500）、热 **140 ms**（门 1,000）、首载 **2,436,583 B**（门 3,145,728，余 709,145）；B2 滚动五轮独立 p95 **16.70/16.80/16.70/16.70/16.70 ms**（门 20）、>50 ms 占 **0.00%**、驱动=重算=呈现 **3601** 五轮全等；排序 p95 **116 ms**、筛选 **32 ms**（门 500）、取消 **14.3 ms**（门 100）、结果与孪生逐项相等；B3 有头五轮 p95 **9.30 ms**（门 20；该机 120 Hz，物理下限 8.33 ms，7,201 呈现/7,201 驱动）、剔除数与孪生相等；**两个负向探针都必败**：B2 301 驱动 302 回调 **0 呈现 0 采样**，B3 601 驱动 602 回调 **601 pump 0 呈现**；B1 回归 冷 188 ms / 热 159 ms / 首载 2,780,592 B / 时延 p95 **45.5 ms**（较 M6 的 46.3 回落，连涨三期终止，余量 4.5 ms）；`cargo test --workspace --lib` 204、`-p data-workbench` 39、`-p xtask` 23；`cd makepad && cargo test` 与脚本 VM 通过；clippy `-D warnings` / fmt 通过；`catalog --check`（20 行）、`css --check`、`sources verify` 通过；本轮证据另见 `docs/validation/p3/review-fixes.md`：浏览器 67/67 + 表格复测 8/8、B2 门 4/4（五轮滚动 p95 16.70–16.80 ms，排序 83 ms、筛选 33 ms、取消 14.1 ms）、Rust 243 通过，原生/wasm Clippy 与静态检查通过；未重跑完整 P3 套件 | `7d4c71d`；评审修复 `dirty@845ed96` |
...
| A-4 | 验收资源 | VoiceOver + Chrome 对 B2/B3 三目标旅程的人工记录；用户 2026-09-11 曾豁免 P2 的 VoiceOver 其余项，本期只要求新增旅程，或由用户再次豁免 | `docs/validation/p3/manual/voiceover-data.md`；M8 | **已解除·用户豁免（2026-09-13）**：用户在被明确问及「跑一遍」还是「豁免」时答「豁免」；七个步骤**未执行**，记为「未测·用户豁免」，不记通过；本期不宣称 B2/B3 的屏幕阅读器支持 |
...
| `crates/rustify-components/src/data_table/{mod.rs,window.rs,keys.rs,view.rs}` ★ | 窗口化表格：行与列都是按槽位复用的元素池（窗口长度处处相同——边缘处缩短会把键盘所在的槽位删掉）、可见范围（含 `MAX_ROWS` 上限——容器没高度时视口会等于整张表，窗口若不封顶会把 externref 表撑爆并以无消息的 `unreachable` 中止）、`role="grid"` 语义、键盘导航（越出窗口时纵横两轴都滚，焦点在窗口重算之后落位）、跳行入口、`window_version` 出口。单元格不带事件监听器：每格两个监听器意味着上千个 wasm 必须持有的 JS 值，focus 与 dblclick 因此上收到网格与行 | M2 |
...
2. 插入/删除：删除先取得当前视图区间对应的数据集位置与稳定 ID，按位置倒序删除；应用改 `Dataset` 与 `version`，同步调用 `Selection::remove_deleted`；当前视图立即做最小修正（删除：从 `order` 去掉该索引并修正后续索引；插入：追加到视图末尾并标陈旧），不等作业；下一次作业以新版本重建。
...
4. 编辑：详情按稳定 ID 解析对象，插入/删除其他行保留该对象和未保存草稿，对象删除则关闭详情。当前行（焦点行或查询定位到的行）的 20 个单元格在右侧详情表单可编辑（复用 P2 表单组件与规则）；提交写回 `cells` 并 `version += 1`（身份不变，但排序/筛选/查找的结果依赖单元格值：写回让当前视图标「陈旧」、让进行中的作业在下一片前失效）；错误经表单错误关联可读。
...
2. 排序：对**当前视图** `order` 的副本做分片归并——第一阶段按预算排若干段（比较 16 字节切片），第二阶段按预算归并；完成后整体替换 `View` 并同步排序标记；筛选提交清除排序标记。取消：票据失效，副本丢弃，旧视图与排序标记不动。A-7 探针⑤在 M1 先测端到端 p95 与片数。**排序排的是视图、筛选换掉视图**：先筛后排保留筛选，先排后筛丢掉排序；两者组合的后果写在 `docs/data.md`。比较是字节序，而样本字母表写作 `A-Z0-9` 却不是它的顺序——数字在字母之前。
3. 筛选：每片在预算内扫描行 × 20 列的子串匹配，命中索引追加到缓冲；完成后替换。空文本条件匹配全部；分组树点击 = 一个带分组谓词的筛选作业，父节点覆盖整组、子节点覆盖该子组。
...
2. 更新：滚动事件或实际 scroller 的 ResizeObserver 尺寸通知 → 重算范围 → 池中每个槽位的 `row_index` 信号更新 → 单元格文本从 `cell(row, col)` 回调重读；DOM 结构不变，只改文本与 `aria-rowindex`/`data-row-id`。列方向同理，可见 12 列加过扫 2 列。尺寸观察器在正常卸载时断开，trap 时由宿主通过实例 AbortSignal 断开，不重入死 wasm。
3. 语义：容器 `role="grid" aria-rowcount=rows+1 aria-colcount=cols`；表头 `role="row" aria-rowindex=1`，数据行 `aria-rowindex=视图位置+2`；单元格 `role="gridcell"`；焦点单元格 `tabindex=0` 其余 `-1`；`aria-selected` 反映选择集；选中隐藏计数在 `aria-live="polite"` 状态行。
4. 键盘：仅 gridcell 处理网格命令，表头按钮保留原生 Enter/Space；滚动回收焦点位置时同步逻辑焦点、实际单元格焦点与唯一单元格 tab 入口，不抢表头或表外焦点。方向键、PageUp/PageDown（一屏）、Home/End（行首尾）、Ctrl+Home/Ctrl+End（首/末行）、Enter/F2（编辑当前行详情）、空格（切换选择）。焦点移到窗口外时，键处理器只记新位置并按需滚动纵横两轴，落焦点由跟随可见范围的效果完成（同一帧内）：槽位显示哪一行是响应式属性，按行号找单元格会找到上一刻的那个；按槽位找到的才是即将带上该行号的元素。表格自己放的焦点不回读——`focusin` 读的正是那批还没写的属性。
...
| M2 | 表格、树与选择 | 前置：M1 已冻结数据集与孪生。**能力目录加两行后目录为 20 类（不再是 R18 的 18 类），四处计数随之更新——`p2-catalog.spec.ts`、`support.ts` 的 `CATALOG_SIZE`、property-workbench 的 `m6-components.spec.ts`、catalog 的 `p2-a11y.spec.ts`——加 catalog 示例的两页，理由记在提交说明里**。`Dataset` 的插入/删除/写回与按行取 ID 在本里程碑随调用方一起加（M1 只交付了有调用方的部分）；`crates/rustify-ui/src/selection.rs` 与单测；`crates/rustify-components/src/data_table/`（`window.rs`/`keys.rs` 单测、行元素池、语义、键盘、跳行入口、`window_version()` 出口）与 `tree.rs`（展开/折叠/方向键/选择事件，筛选作业在 M3 接）；能力目录两行 + `catalog --check`、`css --check`；data-workbench `/table` 视图（表格、树、详情表单含编辑写回与 `version` 递增、概览带 `StripRegion`、插入/删除）；`docs/data.md` 初稿 | V2 全项（只含 M2 交付：跳行三目标、树选择事件、表格侧定位项）；`--project=data-workbench` 绿；一期二期 project 回归绿；回写「实施进度」 |
...
| A-4 VoiceOver 记录 | NFR-4 | 用户提供或豁免 | M8 | 否（**已解除**：用户 2026-09-13 豁免） |
...
- 2 h 耐久要放在一个会话的**最后**，不能跑在回归 sweep 之前：M7 第一次全项回归紧接在 2 h 之后跑，`m8-baseline.spec.ts:102`（一百轮挂载卸载的增长）成了 135 条里唯一的红；同一棵树、隔一段时间重跑就过。增长类断言读的是机器状态和构建两样东西，D13 的独占规则对它同样适用。
- 一个会重复 1,000 次的对照实验要给每轮自己的命名空间：M7 的诊断开销对照同一页跑两遍、每遍有一半动作是重命名，第二遍要的名字第一遍用过，应用正确地拒绝了，而驱动看到的是「一个动作没到达」。另外 `set_diagnostics` 返回的是**被替换掉的旧值**，断言返回值会让一个完全照办了的页面判红——要读 `diagnostics().recording`。
...
| A-4 | §9.4 | 人工记录或豁免 | **已解除·用户豁免（2026-09-13）**，记为未测不记通过 |
