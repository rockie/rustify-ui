...
- 最近更新：2026-09-12（M6 完成并全部跑绿）
- 当前进度：6/8 个里程碑完成
- 当前状态：M1–M6 均已完成并全部跑绿（见完成记录）。M6 把 fusion-basic 的默认挂载换成 B0（10 个 DOM 控件全部来自一期组件 + 一个区域内 20 个 GPU 控件，每类两个 + 一个共享状态结构），页面默认只挂这一个作用域，其余夹具谁要谁挂（`waitForReady` 负责给一期用例挂回两个计数器作用域）；B4 = 两个长期实例各挂一个两区域作用域、只挂卸不重建；`web_gl.js` 记 GPU 字节、`stats().gpu_bytes` 汇总；新增 `p3-{b0,idle,memory}.spec.ts`。**实测**：B0 定义 10/20/1/1 逐项对上、首屏只取一个拉丁字体；B0 峰值 45.0 MiB / GPU 57,576 B，B2 峰值 79.9 MiB / GPU 1,976 B（场景页 286,072 B）；空闲 60 s 呈现 **0** 帧、主线程 0.023% 对空白页 0.011%；隐藏 1 s 内停、60 s 不动，恢复 **10.5 ms**；100 轮隐藏/恢复动作 100/100 无重复；B4 80 轮增长 1.52 MiB、后 40 轮 10,444 B/轮；两实例卸载后 regions/timers/tasks/gpu_bytes/components 全为 0。**`document.hidden` 记未测**（另开标签页置前与 CDP `Page.setWebLifecycleState` 都无法在本无头浏览器上置真，按 §5.6.2 既定退路跳过）。**正文已随实现修订四处**：§3 的 GPU 账本行（口径与不含项）、§5.6 第 2 条（`[hidden]` 会被设了 `display` 的类压过；`document.hidden` 未测）、§5.7 第 3 条（轮次挂的是两区域的计数器作用域；样本在 `collectGarbage` 之后取）、§12 增四条教训
- 最近完成：**M6 · B0、B4、空闲与资源**（2026-09-12）
- 下一步：**M7 · 故障矩阵、长时与诊断开销**——新增 `tests/browser/p3-faults.spec.ts`（4 类 × 20 次：字体 404 经 `serve --fault missing:`、票据反序、pending 中关闭区域、`lose_context`/`restore_context`；每次故障后的已确认状态对照一份预期清单）与 `p3-endurance.spec.ts`（B4 夹具，默认 2 min 回归、`RUSTIFY_ENDURANCE_MINUTES=120` 时 2 h 验收：10 次/s 共 72,000 动作，发出数 = 接受数、0 丢 0 重 0 错，尾部 p95 与头部持平）；R39 AC2 对照 = 诊断开/关各 1,000 次跨区动作、p95 增长 ≤ 5%（P2 的 `m8-baseline.spec.ts` 已有同形夹具可循）；`docs/validation/p3/m7.md` 写入 2 h 曲线。退出条件见 §10 的 M7 行与 §9.1 的 V7。纪律：2 h 那一趟单独跑，不与其他 project 或 cargo 并跑（D13/§12）
- 当前阻塞：无。A-4（VoiceOver 人工记录或用户豁免）仍开放，最晚 M8 确认
- 代码基线：`bb76c29`（M6 的提交；M5 为 `aa0b1e9`，M4 为 `956b89d`，M3 为 `d0a193e`，M2 为 `122a49f`，M1 为 `a6968ca`）
