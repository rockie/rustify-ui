...
| 100 rounds of resource and host checks, no functional leak | `100 mount and dispose rounds return every browser resource`: regions, timers and animation frames return to the baseline, two canvases remain, the error log is empty, no page error, and wasm linear memory is identical at round 20 and round 100. The scope then re-mounts and drives both DOM and GPU again |
...
| wasm linear memory, mount/dispose rounds of a two-region scope | 125,173,760 B at round 10; one allocator step to 142,016,512 B before round 20; identical at every 10-round sample through round 200 |
| Browser resources after 100 rounds | 2 regions, 0 timers, 0 animation frames, 2 canvases, 0 recorded errors |
| Region denied a WebGL2 context | 2 recorded errors for the scope, both its regions `failed`, the other scope untouched |

The memory step between rounds 10 and 20 is a one-time allocator high-water mark, not growth per round: it does not repeat over the next 180 rounds. Linear memory never shrinks, so this is the honest shape to report.
