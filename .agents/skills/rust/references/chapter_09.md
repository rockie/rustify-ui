...
- `Sync` means data can be referenced from multiple threads (equivalently: `&T` is `Send`).

> ❗ These are almost always **conditional on `T`**, not properties of the pointer itself. The column below reads "under what condition on `T`", so a plain "Yes" means the wrapper adds thread-safety on its own.

| Pointer Type | Short Description | Send + Sync? | Main Use |
|---------------- |--------------------------------------------------------------------------- |-------------------------------------- |------------ |
| `&T` | Shared reference | `Send` if `T: Sync`; `Sync` if `T: Sync` | Shared access |
| `&mut T` | Exclusive mutable reference | `Send` if `T: Send`; `Sync` if `T: Sync` | Exclusive mutation |
| `Box<T>` | Heap-allocated owning pointer | `Send` if `T: Send`; `Sync` if `T: Sync` | Heap allocation |
| `Rc<T>` | Single-threaded ref counted pointer | Never - the refcount is non-atomic | Multiple owners (single-thread) |
| `Arc<T>` | Atomic ref counter pointer | `Send + Sync` if `T: Send + Sync` | Multiple owners (multi-thread) |
| `Cell<T>` | Interior mutability without borrow tracking | `Send` if `T: Send`; never `Sync` | Shared mutable, non-threaded |
| `RefCell<T>` | Interior mutability (dynamic borrow checker) | `Send` if `T: Send`; never `Sync` | Shared mutable, non-threaded |
...
| `OnceCell<T>` | Single-thread one-time initialization container (interior mutability ONCE) | `Send` if `T: Send`; never `Sync` | Simple lazy value initialization |
| `LazyCell<T>` | A lazy version of `OnceCell<T>` that calls function closure to initialize | Never `Sync` | Complex lazy value initialization |
| `OnceLock<T>` | Thread-safe version of `OnceCell<T>` | `Send + Sync` if `T: Send + Sync` | Multi-thread single init |
| `LazyLock<T>` | Thread-safe version of `LazyCell<T>` | `Sync` if `T: Send + Sync` | Multi-thread complex init |
...
```rust
let data: String = String::from("this a string");
...
// `&String`, not `&str`: `capacity()` is a `String`-only method.
fn print_capacity(s: &String) {
    println!("{}", s.capacity())
}

// `as_bytes()` exists on `str`, so this one takes `&str`.
fn print_bytes(s: &str) {
    println!("{:?}", s.as_bytes())
}
```

> `print_capacity` is the narrow exception to the "parameters take `&str`, not `&String`" rule from [Chapter 1](./chapter_01.md#11-borrowing-over-cloning): reach for `&String` only when the body needs a method that `str` does not have.
...
```rust
let mut data: String = String::from("this a string");
...
assert_eq!(*x.borrow(), 43, "Not meaning of life plus one");
```
...
let _borrow = x.borrow();

let _mutable = x.borrow_mut(); // 💥 panics: already borrowed
```
> Use `try_borrow`/`try_borrow_mut` when a conflicting borrow is possible and you would rather handle it than crash.

### [`Cell<T>`](https://doc.rust-lang.org/std/cell/struct.Cell.html) - interior mutability without borrow tracking

The fast, panic-free counterpart of `RefCell`: it never hands out a reference to its contents, so there is nothing to track and nothing to panic about. It holds **any** `T` - you move values in and out with `set`, `replace` and `take`. Only `get` requires `T: Copy`, which is why `Cell` is most often seen wrapping small copyable fields:
