...
test.describe("M2 V5: a form that says what is wrong and saves once", () => {
    test("twenty visible properties, each with a label pointing at its control", async ({ page }) => {
    ...
    test("a rule that fails names its field, describes it, and takes the keyboard", async ({ page }) => {
    ...
    test("a rule across two fields belongs to neither and says which pair", async ({ page }) => {
    ...
    test("twenty checks answered backwards leave the latest one showing", async ({ page }) => {
        // Twenty edits, each starting a check. Only the last one is about the
        // value on screen; the others are about values that are gone.
        const edits = rounds(20);
        for (let round = 0; round < edits; round += 1) {
            await type(page, "reference", `REF-${1000 + round}`);
        }
        expect(await checks(page)).toHaveLength(edits);

        // The newest answers first, and it is the one that counts.
        expect(await resolveCheck(page, edits - 1, false)).toBe(true);
        ...
        // Then the stale ones, each with the opposite answer. Not one of them
        // may overwrite what is on screen: they describe values that no
        // longer exist.
        for (let round = edits - 2; round >= 0; round -= 1) {
        ...
        // And a check on a new value still lands.
    ...
    test("asking while a check runs waits, and then saves exactly once", async ({ page }) => {
    ...
    test("a check that fails turns a waiting submit into a block, not a save", async ({ page }) => {
        const before = (await snapshot(page)).form.saves;
    ...
    test("changing a field while a submit waits voids the submit", async ({ page }) => {
        const before = (await snapshot(page)).form.saves;
    ...
    test("twenty clicks while a save runs are one save", async ({ page }) => {
        const before = (await snapshot(page)).form.saves;
    ...
    test("a save that fails keeps the values and says why", async ({ page }) => {
