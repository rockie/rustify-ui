...
pub struct Theme {
    ...
    /// A quiet surface, as the component classes use it. The text drawn on it
    /// is `muted_foreground`; the two were one token until P2's components
    /// needed to say which of them they meant.
    pub muted: Color,
    ...
    /// The surface a panel sits on, and the one a layer floats on. They differ
    /// in the dark theme, where a floating thing has to read as nearer.
    pub card: Color,
    pub popover: Color,
    pub success: Color,
    pub warning: Color,
...
impl Theme {
    ...
    pub fn dark() -> Self {
        Self {
            ...
            secondary: 0x1d2939,
            secondary_foreground: 0xf9fafb,
            muted: 0x161b26,
            muted_foreground: 0x98a2b3,
            accent: 0x32d583,
            accent_foreground: 0x0c111d,
            destructive: 0xf97066,
            destructive_foreground: 0x0c111d,
            card: 0x101828,
            popover: 0x1d2939,
            success: 0x32d583,
            warning: 0xfdb022,
    ...
    pub fn properties(&self) -> [(&'static str, String); 23] {
        [
            ...
            ("--secondary-foreground", hex(self.secondary_foreground)),
            ("--muted", hex(self.muted)),
            ("--muted-foreground", hex(self.muted_foreground)),
            ("--accent", hex(self.accent)),
            ("--accent-foreground", hex(self.accent_foreground)),
            ("--destructive", hex(self.destructive)),
            ("--destructive-foreground", hex(self.destructive_foreground)),
            ("--card", hex(self.card)),
            ("--popover", hex(self.popover)),
            ("--success", hex(self.success)),
            ("--warning", hex(self.warning)),
